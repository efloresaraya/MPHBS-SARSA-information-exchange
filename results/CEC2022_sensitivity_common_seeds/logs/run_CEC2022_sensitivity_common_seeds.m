function run_CEC2022_sensitivity_common_seeds
% =========================================================================
% RUN_CEC2022_SENSITIVITY_COMMON_SEEDS
% =========================================================================
% Final CEC2022 runner for the sensitivity campaign.
%
% Methods
% -------
...
CFG.EXP_NAME      = ['CEC2022_sensitivity_' CFG.VERSION_TAG];
...
anchorIdx  = pick_anchor_index_sarsa_by_meanrank(Entries, AvgRankByMean, Summary);
...
%% ========================= MAIN-TEXT TABLES =========================
export_tables_split(Entries, Pairwise, WTL_global, ...
...
for i = 1:numel(USER_CONFIGS)
    ...
    idx = idx + 1;
    Entries(idx) = make_entry(c.cfg_id, ['PAIR_' char(c.cfg_id) '_' char(c.fad_name)], ...
        'MPHBS_SARSA', 'SARSA', c.label, ...
        CFG.FIXED_K, CFG.FIXED_NI, c.gamma, c.w_best, c.rho_sub, c.fad_name, c.fad_before_p2, c.fad_after_p2, ...
        build_alias('SARSA', c.fad_name, c.cfg_id));
    [Entries(idx).T, Entries(idx).EstimatedFEs] = estimate_T_and_FEs(Entries(idx), CFG);

    idx = idx + 1;
    Entries(idx) = make_entry(c.cfg_id, ['PAIR_' char(c.cfg_id) '_' char(c.fad_name)], ...
        'MPHBS_RANDOM', 'RANDOM', c.label, ...
        CFG.FIXED_K, CFG.FIXED_NI, c.gamma, c.w_best, c.rho_sub, c.fad_name, c.fad_before_p2, c.fad_after_p2, ...
        build_alias('RANDOM', c.fad_name, c.cfg_id));
    [Entries(idx).T, Entries(idx).EstimatedFEs] = estimate_T_and_FEs(Entries(idx), CFG);
end
...
if ~(isfinite(slope) && isfinite(intercept)) || slope <= 0
    error('Invalid FE probe for family=%s, cfg=%s (fe1=%g, fe2=%g).', ...
        entry.family, entry.cfg_id, fe1, fe2);
end
...
function feUsed = probe_actual_fes(entry, CFG, probeDim, Tprobe)
global FE_COUNTER_CEC_SENS_PROBE
FE_COUNTER_CEC_SENS_PROBE = 0;
rng(12345, 'twister');
rawObjective = @(x) sum(x(:).^2);
countedObjective = @(x) probe_count_objective(x, rawObjective);
switch upper(entry.family)
    case 'MPHBS_SARSA'
        MPHBS_main(CFG.PopSize, Tprobe, -100, 100, probeDim, ...
            countedObjective, entry.K, entry.Ni, entry.rho_sub, ...
            entry.w_best, entry.gamma, entry.fad_before_p2, ...
            entry.fad_after_p2);
    case 'MPHBS_RANDOM'
        MPHBS_random_mirror( ...
            CFG.PopSize, Tprobe, -100, 100, probeDim, ...
            countedObjective, entry.K, entry.Ni, entry.rho_sub, ...
            entry.w_best, entry.gamma, entry.fad_before_p2, ...
            entry.fad_after_p2);
    case 'MPHB_BASE'
        MPHB_baseline(CFG.PopSize, Tprobe, -100, 100, probeDim, countedObjective);
    otherwise
        error('Unknown family for FE probe: %s', entry.family);
end
feUsed = FE_COUNTER_CEC_SENS_PROBE;
end

function value = probe_count_objective(x, rawObjective)
global FE_COUNTER_CEC_SENS_PROBE
FE_COUNTER_CEC_SENS_PROBE = FE_COUNTER_CEC_SENS_PROBE + 1;
value = rawObjective(x(:).');
end
...
% Common random numbers across configurations for this function/run block.
...
% ========================================================================
function export_tables_split(Entries, Pairwise, WTL_global, ...
...
idxRANDOM = build_panel_index(Entries, 'RANDOM');

ItemLabels = "F" + string(function_list(:));

SarsaTbl = build_sensitivity_panel_table_unified(Entries, idxSARSA, ItemLabels, 'Function', MeanByF, StdByF, RankByMeanF, AvgRankAll);
RandomTbl = build_sensitivity_panel_table_unified(Entries, idxRANDOM, ItemLabels, 'Function', MeanByF, StdByF, RankByMeanF, AvgRankAll);
StatsTbl = build_anchor_stats_table_unified(Entries, Pairwise, WTL_global, AvgRankAll, anchorIdx);

writetable(AliasTbl,  fullfile(csv_dir, 'cec_alias_map.csv'));
writetable(SarsaTbl,  fullfile(csv_dir, 'cec_sarsa_sensitivity.csv'));
writetable(RandomTbl, fullfile(csv_dir, 'cec_random_sensitivity.csv'));
writetable(StatsTbl,  fullfile(csv_dir, 'cec_anchor_stats.csv'));

write_latex_alias_map_unified(AliasTbl, fullfile(tables_dir, 'cec_alias_map.tex'));
write_latex_sensitivity_panel_unified(Entries, idxSARSA, ItemLabels, 'Function', 'CEC2022', 'error', MeanByF, StdByF, RankByMeanF, AvgRankAll, ...
    'SARSA', fullfile(tables_dir, 'cec_sarsa_sensitivity.tex'));
write_latex_sensitivity_panel_unified(Entries, idxRANDOM, ItemLabels, 'Function', 'CEC2022', 'error', MeanByF, StdByF, RankByMeanF, AvgRankAll, ...
    'RANDOM', fullfile(tables_dir, 'cec_random_sensitivity.tex'));
write_latex_anchor_stats_unified(StatsTbl, string(anchorAlias), fullfile(tables_dir, 'cec_anchor_stats.tex'));
end
