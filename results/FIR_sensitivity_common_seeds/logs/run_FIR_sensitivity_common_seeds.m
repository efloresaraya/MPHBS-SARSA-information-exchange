function run_FIR_sensitivity_common_seeds
% =========================================================================
% RUN_FIR_SENSITIVITY_COMMON_SEEDS
% =========================================================================
% Final FIR runner for the sensitivity campaign.
%
% Methods
% -------
...
CFG.EXP_NAME      = ['FIR_sensitivity_' CFG.VERSION_TAG];
...
USER_CONFIGS = [ ...
    make_cfg('C1', 0.050, 0.30, 0.25, 'BEFORE', true); ...
    make_cfg('C2', 0.025, 0.30, 0.25, 'BEFORE', true); ...
    make_cfg('C3', 0.050, 0.20, 0.25, 'BEFORE', true); ...
    make_cfg('C4', 0.050, 0.30, 0.30, 'BEFORE', true); ...
    make_cfg('C1', 0.050, 0.30, 0.25, 'AFTER',  true); ...
    make_cfg('C2', 0.025, 0.30, 0.25, 'AFTER',  true); ...
    make_cfg('C3', 0.050, 0.20, 0.25, 'AFTER',  true); ...
    make_cfg('C4', 0.050, 0.30, 0.30, 'AFTER',  true) ...
    ];
...
anchorIdx  = pick_anchor_index_sarsa_by_meanrank_fir(Entries, AvgRankAll, Summary);
...
%% ========================= MAIN-TEXT TABLES =========================
idxSARSA = build_panel_index(Entries, 'SARSA');
idxRAND  = build_panel_index(Entries, 'RANDOM');
export_fir_split_tables(Entries, Pairwise, WTL_global, Cases, ...
    idxSARSA, idxRAND, MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll, ...
    anchorIdx, string(anchorAlias), fullfile(CFG.OUTDIR,'tables'), fullfile(CFG.OUTDIR,'csv'));
...
% Common random numbers across configurations for this case/run block.
...
function Entries = build_entries_full(USER_CONFIGS, CFG)
...
idx = 0;
for i = 1:numel(USER_CONFIGS)
    c = USER_CONFIGS(i);
    if ~c.enabled, continue; end
    idx = idx + 1;
    Entries(idx) = make_entry(c.cfg_id, "PAIR_" + c.cfg_id + "_" + c.fad_name, 'MPHBS_SARSA', 'SARSA', c.label, ...
        CFG.FIXED_K, CFG.FIXED_NI, c.gamma, c.w_best, c.rho_sub, c.fad_name, c.fad_before_p2, c.fad_after_p2, idx, build_alias('SARSA', c.fad_name, c.cfg_id));
    idx = idx + 1;
    Entries(idx) = make_entry(c.cfg_id, "PAIR_" + c.cfg_id + "_" + c.fad_name, 'MPHBS_RANDOM', 'RANDOM', c.label, ...
        CFG.FIXED_K, CFG.FIXED_NI, c.gamma, c.w_best, c.rho_sub, c.fad_name, c.fad_before_p2, c.fad_after_p2, idx, build_alias('RANDOM', c.fad_name, c.cfg_id));
end
idx = idx + 1;
...
Kvec = nan(numel(Entries),1); Nivec = nan(numel(Entries),1); Gvec = nan(numel(Entries),1); Wvec = nan(numel(Entries),1); Rvec = nan(numel(Entries),1); Tvec = nan(numel(Entries),1); FEvec = nan(numel(Entries),1); Bvec = nan(numel(Entries),1); Avec = nan(numel(Entries),1);
for i = 1:numel(Entries)
    Kvec(i) = Entries(i).K; Nivec(i) = Entries(i).Ni; Gvec(i) = Entries(i).gamma; Wvec(i) = Entries(i).w_best; Rvec(i) = Entries(i).rho_sub; Tvec(i) = Entries(i).T; FEvec(i) = Entries(i).EstimatedFEs; Bvec(i) = Entries(i).fad_before_p2; Avec(i) = Entries(i).fad_after_p2;
end
T.K = Kvec; T.Ni = Nivec; T.gamma = Gvec; T.w_best = Wvec; T.rho_sub = Rvec; T.fad_before_p2 = Bvec; T.fad_after_p2 = Avec; T.T = Tvec; T.EstimatedFEs = FEvec;
writetable(T, outcsv);
end
...
% ========================================================================
function export_fir_split_tables(Entries, Pairwise, WTL_global, Cases, ...
    idxSARSA, idxRAND, MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll, ...
    anchorIdx, anchorAlias, tables_dir, csv_dir)

AliasTbl = build_alias_table_unified(Entries);
ItemLabels = "C" + string([Cases.id])';

SarsaTbl = build_sensitivity_panel_table_unified(Entries, idxSARSA, ItemLabels, 'Case', MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll);
RandomTbl = build_sensitivity_panel_table_unified(Entries, idxRAND, ItemLabels, 'Case', MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll);
StatsTbl = build_anchor_stats_table_unified(Entries, Pairwise, WTL_global, AvgRankAll, anchorIdx);

writetable(AliasTbl,  fullfile(csv_dir, 'fir_alias_map.csv'));
writetable(SarsaTbl,  fullfile(csv_dir, 'fir_sarsa_sensitivity.csv'));
writetable(RandomTbl, fullfile(csv_dir, 'fir_random_sensitivity.csv'));
writetable(StatsTbl,  fullfile(csv_dir, 'fir_anchor_stats.csv'));

write_latex_alias_map_unified(AliasTbl, fullfile(tables_dir, 'fir_alias_map.tex'));
write_latex_sensitivity_panel_unified(Entries, idxSARSA, ItemLabels, 'Case', 'FIR', 'fitness', MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll, ...
    'SARSA', fullfile(tables_dir, 'fir_sarsa_sensitivity.tex'));
write_latex_sensitivity_panel_unified(Entries, idxRAND, ItemLabels, 'Case', 'FIR', 'fitness', MeanByCaseAll, StdByCaseAll, RankByMeanCaseAll, AvgRankAll, ...
    'RANDOM', fullfile(tables_dir, 'fir_random_sensitivity.tex'));
write_latex_anchor_stats_unified(StatsTbl, string(anchorAlias), fullfile(tables_dir, 'fir_anchor_stats.tex'));
end
