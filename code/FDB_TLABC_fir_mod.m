function out = FDB_TLABC_fir_mod(fobj, cfg)
% FDB-TLABC adapted to FIR with exact FE budget control.
%
% Usage:
%   out = FDB_TLABC_fir_mod(fobj, cfg)
%
% Required:
%   fobj         : function handle, scalar objective on a 1-by-D row vector
...
%
% Output:
%   out.best_f   : best fitness found
%   out.best_x   : best solution
%   out.curve    : best-so-far fitness curve
%   out.fe_used  : exact number of function evaluations used

    if nargin < 2
        error('FDB_TLABC_fir_mod requires fobj and cfg.');
    end
    ...
    for i = 1:popsize
        ...
        val_X(i) = safe_eval_row(fobj, X(i,:));
        FES = FES + 1;
    end
    ...
    while FES < maxFES
        ...
        if ~isempty(ind) && trial(ind) > limit && FES < maxFES
            ...
            for s = 1:nEvalScout
                val_sol(s) = safe_eval_row(fobj, newSol(s,:));
                FES = FES + 1;
            end
...
function cfg = fill_defaults(cfg)
    ...
    if ~isfield(cfg, 'dim')   || isempty(cfg.dim),   cfg.dim   = 31; end
    if ~isfield(cfg, 'maxFE') || isempty(cfg.maxFE), cfg.maxFE = 150000; end
    if ~isfield(cfg, 'lb')    || isempty(cfg.lb),    cfg.lb    = -1; end
    if ~isfield(cfg, 'ub')    || isempty(cfg.ub),    cfg.ub    = 1; end
...
function y = safe_eval_row(fobj, x)
    x = x(:).';
    y = fobj(x);
    if numel(y) > 1
        y = y(1);
